...
# Pines del joystick / movimiento del robot (salidas digitales)
JOY_UP_PIN    = 17   # Adelante (W)
JOY_DOWN_PIN  = 27   # Atrás (S)
JOY_LEFT_PIN  = 22   # Izquierda (A)
JOY_RIGHT_PIN = 23   # Derecha (D)
...
# ================= JOYSTICK / MOVIMIENTO DEL ROBOT =========
# Salidas digitales que controlan el movimiento del robot (adelante, atrás,
# izquierda, derecha). Se controlan desde la interfaz web (joystick + WASD).
JOY_PINS = {
    "up": JOY_UP_PIN,
    "down": JOY_DOWN_PIN,
    "left": JOY_LEFT_PIN,
...
for _joy_pin in JOY_PINS.values():
    GPIO.setup(_joy_pin, GPIO.OUT, initial=GPIO.LOW)

joystick_dirs = set()
last_joystick_time = 0
joystick_lock = threading.Lock()

def set_joystick(dirs):
    """Activa/desactiva los pines GPIO de movimiento según las direcciones dadas.

    'dirs' es un iterable con cualquier combinación de: up, down, left, right.
    Por seguridad se descartan direcciones opuestas simultáneas.
    """
    global joystick_dirs, last_joystick_time
    valid = {d for d in dirs if d in JOY_PINS}

    # Evitar activar pines opuestos al mismo tiempo
    if "up" in valid and "down" in valid:
        valid.discard("up")
        valid.discard("down")
    ...
    with joystick_lock:
        ...
        for name, pin in JOY_PINS.items():
            GPIO.output(pin, GPIO.HIGH if name in valid else GPIO.LOW)

def stop_joystick():
    """Detiene todo el movimiento (todos los pines de movimiento en LOW)."""
    set_joystick(set())

def joystick_watchdog():
    """Detiene los motores si se deja de recibir comandos (pérdida de conexión).

    Mientras haya movimiento activo, el navegador envía un 'keepalive'. Si no
    llega ningún comando en 0.7s, asumimos que se perdió la conexión y paramos.
    """
    while True:
        time.sleep(0.2)
        if joystick_dirs and (time.time() - last_joystick_time) > 0.7:
            stop_joystick()

threading.Thread(target=joystick_watchdog, daemon=True).start()
    ...
    while online:
        try:
            ...
            if cascade is not None and camera_index == 0:  # Solo en cámara 1 para control de servos
                ...
                elif not capture_mode and recognizer is not None and len(faces) > 0:
                    known = False
...
<body>
    <div class="container">
        ...
        <div class="joystick-panel">
            <div class="status-title">Control de Movimiento (Joystick)</div>
            <div class="joystick-wrapper">
                <canvas id="joystick-canvas" width="200" height="200"></canvas>
                <div class="joystick-help">
                    <p>Arrastra el joystick con el ratón o el dedo, o usa las teclas <b>W</b> (adelante), <b>A</b> (izquierda), <b>S</b> (atrás) y <b>D</b> (derecha).</p>
                    <div class="joystick-dirs">
                        <span id="joy-dir-up" class="joy-dir">▲ Adelante</span>
                        <span id="joy-dir-down" class="joy-dir">▼ Atrás</span>
                        <span id="joy-dir-left" class="joy-dir">◀ Izquierda</span>
                        <span id="joy-dir-right" class="joy-dir">▶ Derecha</span>
                    </div>
                    <button class="control-button" onclick="stopJoystick()" style="margin-top: 12px; width: 100%;">
                        Detener Movimiento
                    </button>
                </div>
            </div>
        ...
        <div class="tab-container">
            ...
            <div class="tab-content active" id="info-tab">
                <div style="background: #222222; padding: 20px; border-radius: 8px;">
    ...
    <script>
        ...
        (function() {
            ...
            const cx = W / 2, cy = H / 2;
            const radioBase = 70;   // radio del área de movimiento
            const radioStick = 22;  // radio de la palanca
            const offsetMax = 55;   // desplazamiento máximo del stick
            const umbral = 8;       // zona muerta antes de activar una dirección

            const keyToDir = { w: 'up', s: 'down', a: 'left', d: 'right' };

            let pos = { x: cx, y: cy };
            let dragging = false;
            const keys = new Set();   // teclas WASD presionadas
            let lastSent = null;      // último estado enviado (para evitar repetir)
            let keepalive = null;     // intervalo de keepalive mientras hay movimiento

            function currentDirs() {
                // Combina el arrastre del stick con las teclas WASD
                const dirs = new Set();
                if (dragging) {
                    const dx = pos.x - cx, dy = pos.y - cy;
                    if (dx > umbral) dirs.add('right');
                    else if (dx < -umbral) dirs.add('left');
                    if (dy < -umbral) dirs.add('up');
                    else if (dy > umbral) dirs.add('down');
                }
                keys.forEach(k => { if (keyToDir[k]) dirs.add(keyToDir[k]); });
                return Array.from(dirs);
            }

            function updateIndicators(dirs) {
                ['up', 'down', 'left', 'right'].forEach(d => {
                    const el = document.getElementById('joy-dir-' + d);
                    if (el) el.classList.toggle('active', dirs.indexOf(d) !== -1);
                });
            }
            ...
            function draw() {
                ...
                // Base
                ctx.beginPath();
                ctx.arc(cx, cy, radioBase, 0, Math.PI * 2);
                ...
                ctx.fillText('W', cx, cy - radioBase + 14);
                ctx.fillText('S', cx, cy + radioBase - 14);
                ctx.fillText('A', cx - radioBase + 14, cy);
                ctx.fillText('D', cx + radioBase - 14, cy);

                // Palanca
                ctx.beginPath();
                ctx.arc(pos.x, pos.y, radioStick, 0, Math.PI * 2);
                ctx.fillStyle = dirs.length > 0 ? '#00ff00' : '#ff3333';
                ctx.fill();

                updateIndicators(dirs);
            }

            function send() {
                const dirs = currentDirs();
                const sig = dirs.slice().sort().join(',');
                draw();
                if (sig === lastSent) return;   // sin cambios, no reenviar
                lastSent = sig;
                ...
                }).catch(e => console.error('Error joystick:', e));

                // Keepalive: reenvía el estado mientras haya movimiento, para que
                // el watchdog del servidor no detenga los motores.
                if (dirs.length > 0 && !keepalive) {
                    keepalive = setInterval(() => {
                        fetch('/joystick', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ dirs: currentDirs() })
                        }).catch(() => {});
                    }, 200);
                } else if (dirs.length === 0 && keepalive) {
                    clearInterval(keepalive);
                    keepalive = null;
                }
            }

            // ---- Arrastre con ratón / dedo (pointer events) ----
            function setStickFromPointer(clientX, clientY) {
                const rect = canvas.getBoundingClientRect();
                const x = clientX - rect.left;
                const y = clientY - rect.top;
                const dx = Math.max(-offsetMax, Math.min(offsetMax, x - cx));
                const dy = Math.max(-offsetMax, Math.min(offsetMax, y - cy));
                pos.x = cx + dx;
                pos.y = cy + dy;
                send();
            }
            ...
            function endDrag() {
                ...
                pos.x = cx;
                pos.y = cy;
                send();   // recentra; mantiene direcciones de teclado si las hay
            }
            canvas.addEventListener('pointerup', endDrag);
            canvas.addEventListener('pointercancel', endDrag);

            // ---- Teclado WASD ----
            function moveStickFromKeys() {
                let dx = 0, dy = 0;
                if (keys.has('a')) dx -= offsetMax;
                if (keys.has('d')) dx += offsetMax;
                if (keys.has('w')) dy -= offsetMax;
                if (keys.has('s')) dy += offsetMax;
                pos.x = cx + Math.max(-offsetMax, Math.min(offsetMax, dx));
                pos.y = cy + Math.max(-offsetMax, Math.min(offsetMax, dy));
            }
            ...
            document.addEventListener('keydown', function(e) {
                ...
                if (keyToDir[k] && !keys.has(k)) {
                    keys.add(k);
                    if (!dragging) moveStickFromKeys();
                    send();
                }
            });
            ...
            // ---- Botón / función global para detener ----
            window.stopJoystick = function() {
                keys.clear();
                dragging = false;
                pos.x = cx;
                pos.y = cy;
                lastSent = null;   // fuerza el envío del estado de parada
                send();
            };

            // ---- Seguridad: detener al ocultar / cerrar la página ----
            document.addEventListener('visibilitychange', function() {
                if (document.hidden) window.stopJoystick();
            });
            window.addEventListener('beforeunload', function() {
                try { navigator.sendBeacon('/joystick/stop'); } catch (e) {}
            });
...
def joystick_endpoint():
    """Recibe las direcciones activas del joystick web y mueve el robot.

    Espera un JSON: {"dirs": ["up", "right", ...]}.
    Cada petición refleja el estado completo del joystick (es idempotente),
    por lo que también funciona como 'keepalive' del watchdog de seguridad.
    """
    ...
    if not isinstance(dirs, list):
        dirs = []
    set_joystick(dirs)
    return jsonify({"active": sorted(joystick_dirs)})

@app.route("/joystick/stop", methods=["POST"])
def joystick_stop_endpoint():
    """Detiene el movimiento del robot (todos los pines en LOW)."""
    stop_joystick()
...
def update_person_list():
    ...
    if not persons:
        ...
        for idx, person in enumerate(persons, 1):
            ...
            color = "#00ff00" if person["images"] >= IMAGES_PER_PERSON else "#ff0000"
    ...
    if not online:
        ...
        if not os.path.exists("trainer.yml"):
            messagebox.showwarning("Advertencia", 
                                 ...
                                 "El sistema funcionará solo con detección de color.\n"
                                 "Para reconocimiento facial, entrene el sistema primero.")
            recognizer = None
        else:
            try:
                recognizer = cv2.face.LBPHFaceRecognizer_create()
        ...
        detection_count = 0
        center_pwm()
        status_label.config(text="Estado: ONLINE", foreground="#00ff00")
        ...
        center_pwm()
        face_position = {"x": "center", "y": "center"}
        status_label.config(text="Estado: INACTIVO", foreground="#ff0000")
...
def update_gui():
    ...
    if online:
        status_label.config(text=f"Estado: ONLINE (Cámara Activa: {active_camera_index + 1})", foreground="#00ff00")
        toggle_btn.config(text="DETENER")
    ...
    if recording_cam1:
        recording_label1.config(text="Cámara 1: GRABANDO", foreground="#ff0000")
    else:
        recording_label1.config(text="Cámara 1: Lista", foreground="#00ff00")
    
    if recording_cam2:
        recording_label2.config(text="Cámara 2: GRABANDO", foreground="#ff0000")
    else:
        recording_label2.config(text="Cámara 2: Lista", foreground="#00ff00")
...
# Frame principal de monitoreo que se expande
monitoring_main_frame = tk.Frame(monitoring_tab, bg=bg_color)
monitoring_main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
