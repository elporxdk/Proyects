...
# ================= HTML (diseno de referencia + envio al Arduino) =================
  ...
  (function() {
    ...
    function renderMainMenu() {
      ...
      for (let i = 1; i <= TOTAL_COMPARTMENTS; i++) {
        ...
        if (data && data.completado === true) { badgeClass += ' completed'; badgeText = '\\u2714 Completado'; }
        let previewHtml;
    ...
    function showSavedData(num, data) {
      ...
      document.getElementById('btnEliminar').onclick = function() {
        if (confirm('Eliminar todos los datos del COMPARTIMIENTO ' + num + '?')) { deleteCompData(num); goToMain(); }
      };
      document.getElementById('btnDispensar').onclick = function() { dispensar(num, data); };
    }

    // ---------- DISPENSAR: envia la orden al Arduino por serial (via backend) ----------
    function dispensar(num, data) {
      ...
      .then(r => r.json())
      .then(res => { alert((res.ok ? '\\uD83D\\uDC8A ' : '\\u26A0\\uFE0F ') + res.message); })
    ...
    function handleComplete(num) {
      ...
      record.completado = true; setCompData(num, record); showSavedData(num, record);
    }
